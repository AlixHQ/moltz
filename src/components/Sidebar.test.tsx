...
vi.mock('./SettingsDialog', () => ({
  SettingsDialog: ({ open }: { open: boolean }) => open ? <div data-testid="settings-dialog">Settings Dialog</div> : null,
}));
vi.mock('./SearchDialog', () => ({
  SearchDialog: ({ open }: { open: boolean }) => open ? <div data-testid="search-dialog">Search Dialog</div> : null,
}));
vi.mock('./ExportDialog', () => ({
  ExportDialog: ({ open }: { open: boolean }) => open ? <div data-testid="export-dialog">Export Dialog</div> : null,
}));
...
describe('Sidebar', () => {
  ...
  describe('conversation listing', () => {
    ...
    it('should show selected state for current conversation', () => {
      ...
      const convBtn = screen.getByLabelText(/Conversation: Active Chat \(active\)/);
  ...
  describe('conversation filtering', () => {
    ...
    it('should filter conversations by message content', async () => {
      ...
      store.addMessage(conv1.id, { role: 'user', content: 'Tell me about quantum computing' });
      store.updateConversation(conv1.id, { title: 'Chat 1' });
      
      const conv2 = store.createConversation();
      store.addMessage(conv2.id, { role: 'user', content: 'How to cook pasta' });
