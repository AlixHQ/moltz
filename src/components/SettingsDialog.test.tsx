...
describe('SettingsDialog', () => {
  ...
  describe('Gateway URL field', () => {
    ...
    it('should validate URL format', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/URL must start with ws:\/\/ or wss:\/\//)).toBeInTheDocument();
      });
    });
  ...
  describe('Authentication Token field', () => {
    it('should display token as password by default', () => {
      ...
      const input = screen.getByPlaceholderText(/Stored securely in OS keychain/);
  ...
  describe('Connection Status', () => {
    ...
    it('should test connection when button clicked', async () => {
      ...
      vi.mocked(invoke).mockImplementation((cmd) => {
        ...
        if (cmd === 'connect') return Promise.resolve({ 
          success: true, 
    ...
    it('should show error on connection failure', async () => {
      ...
      vi.mocked(invoke).mockImplementation((cmd) => {
        if (cmd === 'disconnect') return Promise.resolve();
        if (cmd === 'connect') return Promise.reject(new Error('Connection refused'));
        return Promise.reject(new Error('Unknown command'));
      });
  ...
  describe('Default Model selection', () => {
    ...
    it('should group models by provider', () => {
      const { container } = render(<SettingsDialog open={true} onClose={mockOnClose} />);
      
      const optgroups = container.querySelectorAll('optgroup');
      expect(optgroups.length).toBeGreaterThan(0);
      
      const anthropicGroup = Array.from(optgroups).find(g => g.label === 'Anthropic');
  ...
  describe('Thinking toggle', () => {
    it('should display current thinking default state', () => {
      ...
      expect(screen.getByText('Enable Thinking by Default')).toBeInTheDocument();
    });
